...
data class Kkm(
     ...
     *
     * Отсутствие значения остаётся отсутствием: подставленная здесь надпись
     * шла на экран мимо наборов строк — на казахском и английском кассир
     * читал её по-русски, — и попадала в то, по чему ищет список касс:
     * набор «Орг» отбирал все кассы, у которых сведений об организации нет.
     */
    val orgTitle: String?
        get() = ofdServiceInfo?.orgTitle?.takeIf { it.isNotBlank() }
