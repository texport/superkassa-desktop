 ...
 *
 * Совпадение — вхождение в любое из полей поиска без учёта регистра:
 * «касса» найдёт «Суперкасса», а начало кода — модель по коду. Пустой
 * запрос ничего не сужает: набор показывается целиком.
 */
fun <T> narrowed(options: List<T>, query: String, keys: (T) -> List<String>): List<T> {
    val needle = query.trim()
    if (needle.isEmpty()) return options
    return options.filter { option -> keys(option).any { it.contains(needle, ignoreCase = true) } }
}
