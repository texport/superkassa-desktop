package kz.mybrain.superkassa.desktop.ui.strings

internal val moneyTextsKk = MoneyTexts(
    drawer = DrawerTexts(
        inDrawer = "Ақша жәшігінде",
        countedByNode = "Қалдықты касса торабы санайды: алуға ақша жете ме, соны да ол шешеді.",
        unknownBalance = "Тораб қалдықты бермеді — алуды оның өзі тексереді",
        shiftClosed = "Ауысым жабық. Ақша салу мен алу тек ашық ауысымда мүмкін.",
        kkmBlocked = "Касса бұғатталған. Түйін ол бойынша қолма-қол ақша қозғалысын жүргізбейді.",
        shiftClosedShort = "Ауысым жабық",
        amountHint = "Мысалы, 1 500,00",
        notANumber = "Бұл сома емес. Сан енгізіңіз, мысалы 1 500,00",
        notPositive = "Сома нөлден үлкен болуы керек",
        tooLarge = "Сома касса бір операцияда өткізетін шектен асып тұр",
        notEnough = "Жәшікте бар болғаны %s — одан артық алуға болмайды",
        confirmDeposit = "%s салынсын ба?",
        confirmWithdraw = "%s алынсын ба?",
        afterOperation = "Жәшікте %s қалады",
        confirm = "Растау",
        cancel = "Болдырмау",
        working = "Өткізілуде…",
        recent = "Соңғы салымдар мен алулар",
        recentHint = "Соңғы тәуліктегі қолма-қол ақша салу мен алу. Түсімге кірмейді, бірақ жәшіктегі қалдықты өзгертеді — " +
            "жабу кезіндегі салыстыру соларға сүйенеді",
        recentEmpty = "Тәулік ішінде салым да, алу да болған жоқ",
        recentEmptyHint = "Салынған және алынған ақша өткізілген бойда осында көрінеді.",
        recentUnread = "Қолма-қол ақша қозғалысын оқу мүмкін болмады: түйін жауап бермеді.",
        recentUnreadHint = "Оқуды қайталаңыз: түйін үндемей тұрғанда осы тәуліктегі салым мен алу туралы ештеңе белгісіз."
    ),
    cashiers = CashierTexts(
        addTitle = "Жаңа кассир",
        listTitle = "Кассаның кассирлері",
        listHint = "Осы кассаның кассирлері: кім тұра алады және неге рұқсаты бар. Кассир пині — оның чектегі қолы, оны тек " +
            "өзі білуге тиіс",
        pinLength = "4–8 сан",
        pinUnique = "Бір кассадағы екі кассирге бірдей ПИН беруге тораб жол бермейді: " +
            "ол кім жұмыс істеп жатқанын ПИН арқылы таниды.",
        roles = "Әкімші кассирлерді енгізеді және баптауларды өзгертеді, кассир чек шығарады.",
        ownPin = "Өз ПИН-іңіз осында ауысады: жұмыс жаңа ПИН-мен жалғасады, қайта кірудің қажеті жоқ.",
        pinTooShort = "ПИН-де төрт цифрдан аз",
        pinTooLong = "ПИН-де сегіз цифрдан көп",
        nameRequired = "Кассирдің атын жазыңыз",
        deleteConfirm = "%s жойылсын ба?",
        deleteExplain = "Кассир кассаға қолжетімділігін жоғалтады. Ол шығарған чектер орнында қалады.",
        deleteBlocked = "Жалғыз әкімшіні жоюға болмайды: кассирлерді енгізетін және баптауларды өзгертетін ешкім қалмайды. Алдымен екінші әкімшіні енгізіңіз, содан кейін жойыңыз.",
        changePinFor = "%s үшін жаңа ПИН",
        onlyInRole = "Жалғыз әкімші",
        empty = "Кассада бірде-бір кассир енгізілмеген",
        emptyHint = "Алғашқысын енгізіңіз — ол осы тізімде көрінеді.",
        unreadable = "Түйін кассирлер тізімін бермеді",
        unreadableHint = "Баптауларда түйінмен байланысты тексеріп, қайта көріңіз."
    ),
    kkm = KkmSetupTexts(
        stepOne = "1-қадам. Зауыттық нөмір және шығарылған жылы",
        stepTwo = "2-қадам. БФД берген сәйкестендіргіш пен токен",
        copy = "Көшіру",
        copied = "Көшірілді",
        factoryReady = "Нөмір алынды. Оны БФД кабинетіне апарып, сәйкестендіргіш пен токен алыңыз.",
        renameSaved = "Атауы түйінде сақталды",
        renameReset = "Атауды алып тастау",
        diagnosticsHint = "Тексерулер кассада ештеңені өзгертпейді: олар тораб пен БФД-дан жағдайды сұрайды.",
        diagnosticsEmpty = "Тексерулер әлі жүргізілген жоқ",
        programmingOn = "Бағдарламалау режимі қосулы",
        programmingOff = "Қалыпты режим",
        ofdEmpty = "БФД мәлімет жіберген жоқ",
        ofdAnswer = "БФД жауабы",
        ofdKgdNumber = "МКК тіркеу нөмірі",
        ofdFactoryNumber = "Зауыттық нөмір",
        ofdSystemId = "БФД-дағы сәйкестендіргіш",
        ofdProtocol = "Хаттама нұсқасы",
        ofdOrganization = "Ұйым",
        ofdAddress = "Орнату мекенжайы",
        ofdBin = "БСН",
        nodeTitle = "Касса торабы",
        nodeMode = "Жұмыс режимі",
        nodeProtocol = "БФД хаттамасы",
        nodeTimeout = "БФД жауабын күту, с",
        nodeStorage = "Қойма",
        bfdMeaning = "БФД — фискалдық деректер базасы: чектер сонда жіберіледі, касса " +
            "мәліметтері сол жерден келеді.",
        syncTitle = "БФД-мен салыстыру",
        syncService = "Касса мәліметтерін салыстыру",
        syncServiceHint = "Ұйым, мекенжай және тіркеу нөмірлері БФД-дан келеді. " +
            "Ауысым жабық, кезек бос болуы керек.",
        syncServiceDone = "Касса мәліметтері БФД-дан жаңартылды",
        syncCounters = "Есептегіштерді салыстыру",
        syncCountersHint = "Тораб БФД-дан есептегіштер мен ауысым нөмірін алады. Кезек бос болуы керек.",
        syncCountersDone = "Есептегіштер БФД-дан жаңартылды",
        decommission = "Кассаны осы жұмыс орнынан алып тастау",
        decommissionHint = "Түйін кассаны құжаттарымен бірге осы машинадан жояды. КГД-дағы есеп бұл кезде " +
            "өзгермейді: кассаны есептен шығару кабинеттен берілген өтінішпен ғана жасалады. " +
            "Кері қайтаруға болмайды.",
        needProgramming = "Касса бағдарламалау режимінде",
        needShiftClosed = "Ауысым жабық",
        needQueueEmpty = "Жіберу кезегі бос",
        needOnline = "Дербес режим өшірулі",
        decommissionConfirm = "%s кассасы осы машинадан алып тасталсын ба?",
        decommissionDone = "Касса осы жұмыс орнынан алып тасталды"
    )
)
