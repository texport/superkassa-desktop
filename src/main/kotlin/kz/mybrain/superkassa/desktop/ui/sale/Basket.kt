...
class Basket {
     ...
     *
     * Нулевая строка фискального чека — не продажа, и уходить в БФД она
     * не должна: у соседних позиций итог чека остаётся положительным,
     * и общая проверка итога такую строку пропускала.
     *
     * Считается по стоимости строки, а не по цене: цену обнуляет
     * национальный каталог, который цен не несёт, а стоимость — ещё
     * и скидка на позицию, забравшая её целиком. Проверка по одной цене
     * вторую из этих строк в чек пропускала.
     */
    val hasZeroLine: Boolean
        get() = positions.any { it.lineSum.signum() <= 0 }
